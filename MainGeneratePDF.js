...
const REVIEWED_RESPONSES_SPREADSHEET_LINK = "https://docs.google.com/spreadsheets/d/1A2DQyZFzv2JjLolD37xPdFspE3RLenn9pMk544xSZeM/edit#gid=559877815";
...
const BODY = "Dear Inspections Team,<br><br>" +
...
async function Main(e) {
  try {
    const sheet = SpreadsheetApp.openById(RESPONSES_SPREADSHEET_ID).getSheetByName(RESPONSES_SHEET_NAME);

    const headerRowValues = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
    const reviewStatusIndexNo = headerRowValues.indexOf(REVIEWED_STATUS_COLUMN_NAME);
    ...
    const inspectionReportIndexNo = headerRowValues.indexOf(INSPECTION_REPORT_COLUMN_NAME);
    ...
    if (e.range.getColumn() - 1 != reviewStatusIndexNo) {
      ...
      return;
    } else if (e.range.getColumn() - 1 == reviewStatusIndexNo && e.range.getValue() != "Yes"){
    ...
    for (let i = 1; i < sheetValues.length; i++) {
      ...
      newDocLink = "";
      imgBlobs = [];
      if (sheetValues[i][reviewStatusIndexNo] == "Yes" && sheetValues[i][pdfStatusIndexNo] == "") {
        ...
        try {
          ...
          const tempModDocsFolder = DriveApp.getFolderById(TEMP_MOD_DOCS_FOLDER_ID);
          ...
          console.log(e);
          sendErrorEmail(e);
          sheet.getRange(i + 1, pdfStatusIndexNo + 1).setValue("PDF Not Generated");
          ungeneratedDocsCount++;
          continue;
        }
        try {
          console.log("PDF Generation Started for data in row " + i);
          sheet.getRange(i + 1, pdfStatusIndexNo + 1).setValue("Generating PDF");
          await updateDocToPdf(doc, inspRepoUrls, i);

          if (errorMailSent == false) {
            sheet.getRange(i + 1, pdfStatusIndexNo + 1).setValue("PDF Generated");
            sheet.getRange(i + 1, modDocIndexNo + 1).setValue(newDocLink);
            sheet.getRange(i + 1, pdfLinkIndexNo + 1).setValue(pdfFileLink);

            const warehouseCode = sheetValues[i][headerRowValues.indexOf(WAREHOUSE_CODE)];
            const auditorName = sheetValues[i][headerRowValues.indexOf(AUDITOR_NAME)];
            const timestamp = sheetValues[i][headerRowValues.indexOf(TIMESTAMP)];
            const attachments = [DriveApp.getFileById(await extractFileIdFromLink(pdfFileLink)).getBlob()];
            ...
            await sendEmail(RECIPIENTS, CC_RECIPIENTS, BCC_RECIPIENTS, SUBJECT, BODY, attachments, bodyTags, subjectTags);
            console.log("PDF Generation Notification mail sent");

            generatedDocsCount++;
          }
          else {
            if (sheet.getRange(i + 1, pdfStatusIndexNo + 1).getValue().toString() != "PDF Not Generated. Temporary Network Problem. Please Try Again.") {
              sheet.getRange(i + 1, pdfStatusIndexNo + 1).setValue("PDF Not Generated");
            }
            ungeneratedDocsCount++;
          }
        } catch (error) {
          const e = "Error updating doc to pdf: " + doc.getName() + " " + error.stack;
          console.log(e);
          sendErrorEmail(e);
        }
      }
